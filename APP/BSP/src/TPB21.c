...
void TPB21_Process(void)                         //TPB21主进程
{
  ...
  {
    memset(TPB21.R_Buffer,'\0',RECV_BUFF_SIZE);//清缓存
    TPB21.Recv_Length = Uart2_Receive((unsigned char *)TPB21.R_Buffer);//接收数据
    
    switch(TPB21.Start_Process)
    {
    case NRB:                  //重启
      {
        if(strstr(TPB21.R_Buffer,"REBOOT_CAUSE_APPLICATION_AT") != NULL)
        {
    ...
    case GETNBAND:               //查询频段
      {       
        if(strstr(TPB21.R_Buffer,"+NBAND:5") != NULL)    //支持频段5
        {
    ...
    case GETCFUN:                //查询电话功能
      {
        if(strstr(TPB21.R_Buffer,"+CFUN:1") != NULL)     //全功能
        {         
    ...
    case GETCGATT:       //查询网络激活状态
      {
        if( strstr(TPB21.R_Buffer,"+CGATT:1") != NULL)//网络激活
        {        
    ...
    case CEREG:       //查询网络注册状态
      {
        if( strstr(TPB21.R_Buffer,"+CEREG:0,1") != NULL)//网络注册
        {        
    ...
    case  GETNCDP:                 //查询CDP服务器
      {
        if( strstr(TPB21.R_Buffer,"+NCDP:180.101.147.115,5683") != NULL)//获取到NCDP
        {        
    ...
    case MLWULDATA:        //发送消息
      {  
        ...
        { 
          ...
          if(strnstr(str1,"+MLWDLDATA:4,AAAA0000",24) != NULL)  //上报全部参数的响应
          {   
            TPB21.Report_Bit = 2;
          ...
          else if(strnstr(str1,"+MLWDLDATA:4,AAAA0001",24) != NULL)     //上报历史流量的响应
          {
            TPB21.Report_Bit = 3;
          ...
          else if(strnstr(str1,"+MLWDLDATA:4,AAAA0002",24) != NULL)     //上报历史数据的响应
          {
          ...
          else if(strnstr(str1,"+MLWDLDATA:24,02",24) != NULL)     //设置基础参数命令
          {
            Recv_Data_Process((unsigned char*)str1);
          }
          else if(strnstr(str1,"+MLWDLDATA:8,04",24) != NULL)     //校表命令
          {
            Recv_Data_Process((unsigned char*)str1);
          }
          else if(strnstr(str1,"+MLWDLDATA:17,06",24) != NULL)     //校时命令
          {
            Recv_Data_Process((unsigned char*)str1);
          } 
